...
This package provides three small Python scripts that work together:
...
- **`gate_bot.py`** > a Telegram bot that opens the gate when an authorized user sends a keyword, and notifies the other authorized users that the gate was opened.
...
Edit `gate_bot.py` and set:

- `TELEGRAM_TOKEN` — your bot token
- `AUTHORIZED_CHAT_IDS` — list of user chat IDs allowed to open the gate (get yours from [@userinfobot](https://t.me/userinfobot))
- `KEYWORDS` — messages that trigger the gate (`apri`, `open`, `🚪`, …)
- `CODICE_CANCELLO` — the key in `broadlink_codes.json` for the gate code
...
Authorized users can write any of the keywords and the bot opens the gate. When one user opens it, the others receive a notification with the username and timestamp.

### Security considerations

A gate is a physical-security control. Treat the bot token and chat-id whitelist accordingly:

- Only users in `AUTHORIZED_CHAT_IDS` can trigger the gate. Everyone else gets a refusal and the attempt is logged.
...
- Never commit your bot token or `broadlink_codes.json` to git. The included `.gitignore` already excludes them.
